...
const TableTrip = ({ onEdit }) => {
  const [data, setData] = useState([]);
  useEffect(() => {
    const token = localStorage.getItem('token'); // Substitua pelo seu token real
    const apiUrl = 'http://20.226.34.88:5000/config/get_trip';
    function update() {
      fetch(apiUrl, {
        headers: {
  ...
  const handleOnDelete = async (data) => {
    const id = data.id;
    try {
      const response = await fetch('http://20.226.34.88:5000/delete_trip/' + id, {
        method: 'POST'
      });
    } catch (error) {
    ...
    () => [
      {
        ...
        accessor: 'placa',
        Cell: ({ row }) => {
          const { origem, destino } = row.original;
          return (
            <div className='infos_row'>
              <span>{origem}</span>
              <span>{destino}</span>
